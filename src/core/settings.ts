...
export const DEFAULT_SETTINGS: AssistantConfig = {
  ...
  voiceEnabled: true, voiceName: '', voiceRate: 0.98, voicePitch: 0.92, voiceVolume: 1, voiceLanguage: 'en-US', speechProvider: 'browser', groqTtsVoice: 'troy', groqTtsModel: 'canopylabs/orpheus-v1-english', customVoiceFileName: '', customVoiceUrl: '',
...
export function normalizeSettings(input: Partial<AssistantConfig>): AssistantConfig {
  ...
  s.speechProvider = s.speechProvider === 'groq' ? 'groq' : 'browser'; s.groqTtsVoice = String(s.groqTtsVoice || DEFAULT_SETTINGS.groqTtsVoice).trim().slice(0, 80); s.groqTtsModel = String(s.groqTtsModel || DEFAULT_SETTINGS.groqTtsModel).trim().slice(0, 120);
