...
const port=Number(process.env.PORT??8787); const dbPath=process.env.DATABASE_PATH??'./data/jarvis.sqlite';
mkdirSync(dbPath.substring(0,Math.max(0,dbPath.lastIndexOf('/')))||'.',{recursive:true});
const store=new Store(dbPath); const roots=(process.env.ALLOWED_FILESYSTEM_ROOTS??'./workspace').split(',').map(s=>s.trim()).filter(Boolean);
const tools=new ToolRegistry(store,roots); registerBuiltIns(tools); const llm=new StubLLM(); const bus=new EventBus();
const app=express(); app.use(express.json({limit:'2mb'})); app.use(express.static('public')); app.get('/',(_req,res)=>res.sendFile('index.html',{root:'public'}));
const settingsFor=(userId:string)=>store.getSettings(userId);
const agentFor=(userId:string)=>new Agent(store,tools,llm,settingsFor(userId));
...
app.get('/api/settings',(req,res,next)=>{try{const userId=String(req.query.userId??'default-user');res.json(settingsFor(userId));}catch(err){next(err)}});
app.put('/api/settings',(req,res,next)=>{try{const userId=String(req.body?.userId??'default-user'); const patch={...req.body}; delete patch.userId; const settings=store.saveSettings(userId,normalizeSettings(patch)); res.json({ok:true,settings});}catch(err){next(err)}});
...
app.post('/api/assistant/message',async(req,res,next)=>{try{const body=z.object({userId:z.string().min(1),conversationId:z.string().min(1),message:z.string().min(1).max(20000)}).parse(req.body);const result=await agentFor(body.userId).handle(body.userId,body.conversationId,body.message);await bus.emit({type:'USER_MESSAGE_RECEIVED',source:'api',timestamp:new Date().toISOString(),content:{conversationId:body.conversationId}});res.json(result)}catch(err){next(err)}});
